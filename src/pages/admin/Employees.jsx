...
const Employees = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState(['Employee ID', 'Date of Joining', 'Employee Rating', 'Comments', 'Manager ', 'Access Roles', 'Email Address', 'Contact Number', 'Teams', 'Alternative Mobile']);
  ...
  const [formData, setFormData] = useState({
    employeeId: '', dateOfJoining: '', employeeRating: '', comments: '', manager: '', accessRoles: '', emailAddress: '' , contactNumber: '' , alternativeMobile: ''
  });
 ...
 const filtered = employees.filter((row) =>
  ...
  (filterType === '' || row.accessRoles === filterType)
);
  ...
  const handleDialogOpen = (type) => {
    if (type === 'edit') {
      ...
      setFormData({ employeeId: '', dateOfJoining: '', employeeRating: '', comments: '', manager: '', accessRoles: '', emailAddress: '' , contactNumber: '' , alternativeMobile: '' });
    }
    setDialogType(type);
    setDialogOpen(true);
  };
...
const handleReset = () => {
  setVisibleColumns(['Employee ID', 'Date of Joining', 'Employee Rating', 'Comments', 'Manager ', 'Access Roles', 'Email Address', 'Contact Number', 'Teams', 'Alternative Mobile']);
  setHiddenColumns([]);
};
...
const handleImportSubmit = () => {
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: '#fff', borderRadius: 2 }}>
      <Breadcrumbs current="Employees" />
      {/*  Heading */} 
      <Typography variant="h6" sx={{ mb: 2 }}>Employees</Typography>
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mb: 2 }}>
  {/* FILTERS ROW: Start Date, End Date, Select Report */}
  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap',padding: '4px 10px' }}>
    <TextField
      label="Start Date"
      type="date"
      size="small"
      InputLabelProps={{ shrink: true }}
      sx={{ minWidth: 140, ...smallerInputSx }}
    />
    <TextField
      label="End Date"
      type="date"
    ...
    <FormControl size="xsmall" sx={{ minWidth: 160, ...smallerInputSx }}>
      ...
      <Select value="" label="Select Report"   MenuProps={{
          PaperProps: {
      ...
      >
        <MenuItem value=""><em>None</em></MenuItem>
         <MenuItem value="enquiryMis">Enquiry MIS Report</MenuItem>
    <MenuItem value="activityMis">Activity MIS Report</MenuItem>
      </Select>
    </FormControl>
  </Box>

  {/* BUTTON ROW: Left (Modify/Delete), Right (Search + Icons) */}
  <Box sx={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 1 }}>
    {/* LEFT SIDE: Modify | Delete | Batch Update */}
    ...
    {/* RIGHT SIDE: Select Action + Search + Query + Icons + Create */}
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
    {/* Select Action */}
    <FormControl size="xsmall" sx={{ minWidth: 160, ...smallerInputSx }}>
      <InputLabel>Select Action</InputLabel>
      <Select value="" label="Select Action"  MenuProps={{
          PaperProps: {
            sx: { '& .MuiMenuItem-root': { fontSize: '0.75em' } }
          }
        }}
       >
        <MenuItem value=""><em>None</em></MenuItem>
    <MenuItem value="viewAccessRules">View Access Rules</MenuItem>
    <MenuItem value="viewProjectFlowRules">View Project Flow Rules</MenuItem>
    <MenuItem value="viewWorkFlowRules">View Work Flow Rules</MenuItem>
      </Select>
    </FormControl>
    ...
    <FormControl size="xsmall" sx={{ minWidth: 160, ...smallerInputSx }}>
      ...
      {[...new Set(data.map((item) => item.accessRoles).filter(Boolean))].map((role) => (
  <MenuItem key={role} value={role}>{role}</MenuItem>
    ...
    {/* Table View Icon */}
    <IconButton size="small" title="Column Preferences" onClick={() => setColumnDialogOpen(true)} sx={{ color: 'grey' }}>
      <TableViewIcon />
    </IconButton>
    ...
    <IconButton
      ...
      onClick={() => {
        const rows = data.map(d => [
          d.employeeId, d.dateOfJoining, d.employeeRating, d.comments || '', d.manager || '', d.accessRoles || '', d.emailAddress || '', d.contactNumber || '', d.teams || '', d.alternativeMobile || '', d.created || '', d.createdBy || '', d.lastUpdatedBy || '', d.lastUpdated || ''
        ]);

        const sheet = XLSX.utils.aoa_to_sheet([
          ['Employee ID', 'Date of Joining', 'Employee Rating', 'Comments', 'Manager ', 'Access Roles', 'Email Address', 'Contact Number', 'Teams', 'Alternative Mobile', 'Created', 'Created By', 'Last Updated By', 'Last Updated'],
          ...rows
        ]);

        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, sheet, 'Employee_data');
    ...
    >
      ...
      <TableContainer component={Paper}>
        <Table size="small">
          ...
          { key: 'employeeRating', label: 'Employee Rating' },
            ...
            {paginated.length > 0 ? (
              paginated.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell sx={{ fontSize: '0.75rem' }}>{row.employeeId}</TableCell>
                  <TableCell sx={{ fontSize: '0.75rem' }}>{row.dateOfJoining}</TableCell>
                  <TableCell sx={{ fontSize: '0.75rem' }}>{row.employeeRating}</TableCell>
                  <TableCell sx={{ fontSize: '0.75rem' }}>{row.comments}</TableCell>
                  <TableCell sx={{ fontSize: '0.75rem' }}>{row.manager}</TableCell>
                  <TableCell sx={{ fontSize: '0.75rem' }}>{row.accessRoles}</TableCell>
                  <TableCell sx={{ fontSize: '0.75rem' }}>{row.emailAddress}</TableCell>
                   <TableCell sx={{ fontSize: '0.75rem' }}>{row.contactNumber}</TableCell>
                    <TableCell sx={{ fontSize: '0.75rem' }}>{row.teams}</TableCell>
                     <TableCell sx={{ fontSize: '0.75rem' }}>{row.alternativeMobile}</TableCell>
                </TableRow>
              ))
            ) : (
  ...
  <TextField
    ...
    onChange={(e) => setFormData({ ...formData, employeeRating: e.target.value })}
    sx={{
      fontSize: '0.75rem',
      '& .MuiInputBase-input': { fontSize: '0.75rem' },
      '& .MuiInputLabel-root': { fontSize: '0.75rem' }
  ...
  <TextField
    ...
    onChange={(e) => setFormData({ ...formData, accessRoles: e.target.value })}
    sx={{
      fontSize: '0.75rem',
      '& .MuiInputBase-input': { fontSize: '0.75rem' },
      '& .MuiInputLabel-root': { fontSize: '0.75rem' }
